<?php
$activeRoute = 'asm_per_area';
$rows = $rows ?? [];
?>

<div class="space-y-5 manager-page">
    <div>
        <h1 class="text-3xl font-extrabold text-sky-600 tracking-tight uppercase">ASM PER AREA</h1>
        <p class="text-lg text-slate-700 mt-1">Assigning of Area Sales Manager per Municipality</p>
    </div>

    <div class="manager-table-card">
        <div class="manager-table-head">
            <h2 class="manager-table-title">Manage ASM Area</h2>
            <div class="manager-table-controls">
                ...
                <button type="button" class="manager-add-btn" aria-label="Add ASM per area" onclick="openAddAsmPerAreaModal()">
                    <span class="manager-add-icon">+</span>
                    <span>Add ASM Area</span>
                </button>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="manager-table-grid">
                ...
                <tbody>
                    <?php if (empty($rows)): ?>
                        <tr>
                            <td colspan="7" class="text-center text-slate-500">No ASM per area records found.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            ...
                            $initial = strtoupper(substr((string)($row['sales_manager'] ?? 'U'), 0, 1));
                            ?>
                            <tr>
                                <td>
...
<template id="add-asm-per-area-modal-template">
    <div class="manager-modal">
        <div class="manager-modal-header">
            <h3>Add ASM Per Area</h3>
            <button type="button" class="manager-modal-close" onclick="closeAddAsmPerAreaModal()" aria-label="Close">x</button>
        </div>

        <form class="manager-modal-form" method="POST" action="<?= App\Config\App::url('asm/per-area') ?>">
            <?= \App\Helpers\Csrf::field(); ?>
            <div class="manager-modal-grid">
                <label class="manager-modal-field">
                    <span>Sales Manager</span>
                    <input type="text" name="sales_manager" placeholder="Enter Sales Manager" required>
                </label>
                <label class="manager-modal-field">
                    <span>Region</span>
                    <select name="region" required>
                        <option value="">-- SELECT REGION --</option>
                        <option value="NCLZ">NCLZ</option>
            ...
            <div class="manager-modal-actions">
                ...
                <button type="button" class="manager-modal-cancel" onclick="closeAddAsmPerAreaModal()">Cancel</button>
            </div>
        </form>
    </div>
</template>
...
<script>
    function openAddAsmPerAreaModal() {
        const template = document.getElementById('add-asm-per-area-modal-template');
    ...
    function closeAddAsmPerAreaModal() {
        const content = document.getElementById('modal-content');
