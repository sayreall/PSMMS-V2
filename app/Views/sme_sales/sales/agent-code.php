...
<div class="space-y-5 manager-page">
    <div>
        <h1 class="text-3xl font-extrabold text-sky-600 tracking-tight uppercase">Agent Code</h1>
        <p class="text-lg text-slate-700 italic mt-1">The agent code is for sub-agents and partners</p>
    </div>

    <div class="manager-table-card">
        <div class="manager-table-head">
            <h2 class="manager-table-title">Manage Agent</h2>
            ...
            <div class="manager-table-controls">
                ...
                <button type="button" class="manager-control-icon" aria-label="Search">
                    <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 0 0114 0z"/></svg>
                </button>
                <button type="button" class="manager-add-btn" aria-label="Add Agent Code" onclick="openAddAgentCodeModal()">
                    <span class="manager-add-icon">+</span>
                    <span>Add Agent Code</span>
                </button>
            </div>
        </div>
...
<template id="add-agent-code-modal-template">
    <div class="manager-modal">
        <div class="manager-modal-header">
            <h3>Add Agent Code</h3>
            <button type="button" class="manager-modal-close" onclick="closeAgentCodeModal()" aria-label="Close">x</button>
        </div>

        <form class="manager-modal-form" method="POST" action="<?= App\Config\App::url('agent-code') ?>">
            ...
            <div class="manager-modal-grid">
                ...
                <label class="manager-modal-field">
                    ...
                    <input type="text" name="agent_code" placeholder="Enter Agent Code">
                </label>
            </div>

            <div class="manager-modal-actions">
                <button type="submit" class="manager-modal-submit">Add</button>
                <button type="button" class="manager-modal-cancel" onclick="closeAgentCodeModal()">Cancel</button>
            </div>
        </form>
    </div>
</template>
...
<script>
    function openAddAgentCodeModal() {
        const template = document.getElementById('add-agent-code-modal-template');
